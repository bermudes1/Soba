...
int testa_certo(BigInt a, BigInt b, char *msg, int test) {
   ...
   /*contadora para erros*/

   int erro = 0;

   /*Percorre os arrays BigInt e compara cada byte */
   
   for (int i = 0; i < (NUM_BITS/8); i++){

    /*Em caso de bytes diferentes, aponta o erro e define erro = 1 */
   ...
   /* Se teve erro, exibe mensagem de onde está o erro */
...
int teste_da_big_val (char*msg) {
   ...
   /*contadora de erros (dos teste)*/

   int erros = 0;

   /*testa extensão de um número positivo (1)*/

   BigInt resp1 = {0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

   /*vai guarda o valor de todas as chamadas da função*/

   BigInt res;
   big_val (res, 1);

   /*se tiver erros, adiciona 1 no contador*/
   
   if (testa_certo(res, resp1, msg, 1) == 1){
      erros += 1;
   } 

   /*testa a extensão de um número negativo (-2)*/


   BigInt resp2 = {0xFE, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
   big_val (res, -2);

   /*Se tiver erro, adicona 1 ao contador*/
   ...
   /*Retorna 1 (False) se tivemos erros, caso contrario, retorna 0 (True)*/
...
int teste_da_big_comp2(char*msg) {
   ...
   /*guarda os resultados gerados por cada chamada de função para número positivo */

   BigInt res,res1;

   /*testa o complemento a 2 de um número positivo (5) */

   /*logica:A estratégia de teste consiste em observar o valor de entrada e o resultado 
   esperado da conversão de long para BigInt para então comparar os arrays dos números 
   estendidos. abordagem padrão que usamos em todo o programa de testes.*/
   ...
   /*Retorna 1 (False) se tivemos erros, caso contrario, 0 (True)*/
...
int teste_da_big_sum(char*msg) {
   ...
   /*guarda a extensão de cada número a ser usado nos testes */

   BigInt a,b,c,d,pos,grande,f;

   /*guarda os resultados gerados por cada chamada de função */
...
int teste_da_big_mul(char*msg){
   ...
   /*guarda os resultados esperados */
   ...
   /*Extendendo os valores a serem usados: 
    Números positivo
    negativo
    zero
    muito grande (com overflow)*/
...
int teste_da_big_shl(char*msg){
   ...
   /*guarda a extensão de cada número a ser usado nos testes (Números positivo e negativo) */

   BigInt a,b;

   /*guarda os resultados esperados para números positivos */

   BigInt res,res1,res2,res3;

   /*guarda os resultados das chamadas da função */

   BigInt pos,pos1,pos2,pos3;

   /*guarda os resultados esperados para números negativos */
   ...
   /* Testes do número positivo (Shift de 0 bits para) */
   ...
   /* Testes do número negativo (Shift de 0 bits) */
...
int teste_da_big_shr(char*msg){
   ...
   /*guarda o retorno esperado para cada chamada de função (Números positivos) */
   ...
   /*guarda o resultado esperado para números negativos */

   BigInt neg;

   /*guarda o retorno esperado para cada chamada de função (Números negativos) */
   ...
   /* Testes para número positivo (Shift de 0 bits)*/
...
int teste_da_big_sar(char*msg){
   ...
   /*usamos para extensão de cada número a ser usado nos testes (números positivo e negativo) */

   BigInt a,b;

   /*guarda o resultado da chamada da função para cada shift de número negativo */
...
int main() {
   ...
   /*somamos o retorno de cada func em erros*/
